...
flags.DEFINE_string(
    'train_dir', 'train_title.tsv',
    'train_dir for training.'
)

flags.DEFINE_string(
    'dev_dir', 'dev.tsv',
    'dev_dir for evaluation.'
)

flags.DEFINE_string(
    'test_dir', 'test_title.tsv',
    'test_dir for testing.'
)
...
flags.DEFINE_string(
    "init_checkpoint",
    './data/pretrained_model/uncased_base/bert_model.ckpt',
    "Initial checkpoint (usually from a pre-trained BERT model).")

flags.DEFINE_string(
    'eval_checkpoint_path',
    './data/sentiment/output/model.ckpt',
    'eval_checkpoint_path used in evaluation.')

flags.DEFINE_string(
    'eval_result_filename',
    'eval_results.txt',
    'name of file eval results stored in under output_dir.'
)
...
flags.DEFINE_integer('keep_checkpoint_max', 20, 'how many checkpoint files to keep.')

flags.DEFINE_integer('save_summary_steps', 100, 'how many steps to draw summary in tensorboard.')
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  if FLAGS.classifier == 'MLP':
    ...
    output_layer = tf.layers.dense(tf.reshape(hidden_layer, tf.convert_to_tensor([-1, hidden_layer.shape[-1].value])),
...
def main(_):
  ...
  processors = {"sentiment": SentimentProcessor}
  ...
  if FLAGS.do_eval:
    ...
    result = estimator.evaluate(input_fn=eval_input_fn, steps=eval_steps, checkpoint_path=FLAGS.eval_checkpoint_path)

    output_eval_file = os.path.join(FLAGS.output_dir, FLAGS.eval_result_filename)
  ...
  if FLAGS.do_predict:
    ...
    result = estimator.predict(input_fn=predict_input_fn, checkpoint_path=FLAGS.test_checkpoint_path)

    output_predict_file = os.path.join(FLAGS.output_dir, "test_results_only_prob.tsv")
    output_predict_final_dir = os.path.join(FLAGS.output_dir, "submission.csv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
        with tf.gfile.GFile(output_predict_final_dir, 'w') as writer_sub:
            tf.logging.info("***** Predict results *****")
            df = pd.DataFrame(pd.read_csv(os.path.join(FLAGS.data_dir, FLAGS.test_dir), sep='\t', header=None))
            df.columns = ['label', 'id', 'context']
            id = df['id']
            writer_sub.write('id' + ',' + 'label' + '\n')
            i = 0
            for prediction in result:
                ...
                output_line_sub = id[i] + ',' + str(np.argmax(prediction)) + '\n'
                writer_sub.write(output_line_sub)
                i += 1
